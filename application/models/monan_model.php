...
class monan_model extends CI_Model implements crudmodel{
	public function getDetail($id = ''){
		$this->load->database();
		$query = $this->db->get_where('monan', array('id' => $id));
	...
	public function countRecord() {
		$this->load->database();
		$sql = 'SELECT COUNT(id) as count FROM monan';
	...
	public function getTop($column, $start, $n) {
		...
		$this->load->database();
		$sql = "SELECT id, ten, anh_dai_dien, tom_tat, viewcount, rate FROM monan ORDER BY $column DESC, $column2 DESC LIMIT ?, ?";
	...
	public function insert($obj) {
		...
		$obj->danh_sach_nha_hang=json_encode($obj->danh_sach_nha_hang);
		$data = (array)$obj;
		$this->load->database();
		$this->db->insert('monan', $data);
		return mysql_insert_id();
	}
		
	public function delete($id) {
		$this->db->delete('monan', array('id' => $id));
	}
